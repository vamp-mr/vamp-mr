...
### Multi-Robot Collision Support

The header `vamp/collision/multi_robot.hh` provides `fkcc_multi_*` helpers for checking multiple traced robots at once. Build a state with `vamp::collision::make_multi_robot_state<Robot, rake>(configuration, base_transform, optional_attachment)` for each robot and call one of:

- `fkcc_multi_all` / `fkcc_multi_all_attach` – check robots (and optional attachments) against each other **and** a shared environment. A collision filter can be supplied to selectively ignore link pairs (see below).
- `fkcc_multi_self` / `fkcc_multi_self_attach` – check only the inter-robot contacts (environment obstacles ignored).

Each helper internally runs the robot's traced `fkcc` / `fkcc_attach`, applies the requested global transforms, and then performs pairwise sphere tests between robots and attachments. Python bindings are available via `vamp.fkcc_multi_all(states, environment, allowed_collisions=None)` and `vamp.fkcc_multi_self(states, allowed_collisions=None)`, where each state dictionary supplies a `robot` name, raw joint `configuration`, and an optional `transform`/`attachment`. When `allowed_collisions` is provided, it must be an iterable of mappings such as `{"state": 0, "link": "link_2", "object": "table", "enabled": False}`; entries whose `enabled` (or `collide`) flag evaluates to `False` allow the specified link to contact the named environment object for that state. To permit tool/object attachments to contact the environment, set `"attachment": True` in the mapping (for example `{"state": 1, "attachment": True, "object": "bin", "enabled": False}`). See `scripts/cpp/multi_robot_example.cc` and `scripts/multi_robot_pybullet_compare.py` for executable examples.
